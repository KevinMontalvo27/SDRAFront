    ...
    <nav class="flex flex-col h-full bg-base-100">
      ...
      <div class="p-4 border-b border-base-300 flex items-center justify-between gap-3">
        <div class="flex items-center gap-3">
          ...
          <span class="text-lg font-bold">{{ cursoNombre }}</span>
        </div>

        <!-- Botón cerrar sidebar -->
        <button
          (click)="onClose && onClose()"
          class="w-10 h-10 flex items-center justify-center rounded-full text-2xl text-base-content/60 hover:text-base-content hover:bg-base-200 transition-colors"
          title="Ocultar menú"
        >
          «
        </button>
